  ...
  <div class="card">
    <div class="griglia2">
      ...
      <div><label class="lab">Settimane</label>
        <div class="segm" style="margin-top:6px">
          <button onclick="setPianoSett(-4)">−4</button>
          <div style="flex:1;text-align:center;align-self:center;font-weight:600">${p.nSettimane}</div>
          <button onclick="setPianoSett(4)">+4</button></div></div>
    </div>
...
function setPianoSett(d) {
  const p = pianoDati();
  p.nSettimane = Math.max(4, Math.min(52, p.nSettimane + d));
  while (p.righe.length < p.nSettimane) p.righe.push({ fase: "", blocco: "", sist: "", ciclo: "" });
